...
export default function SettingsClient({
  ...
  // Initialize from agent (safe defaults)
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    setMsg(null);
    const r = await fetch(`/api/a/${agentId}/settings`, {
      method: "POST",
  ...
  return (
    <main style={{ maxWidth: 720, margin: "40px auto" }}>
      ...
      <p>
        Quick links:{" "}
        {firstForm && <a href={`/form/${firstForm.id}`}>Form</a>}{" · "}
        <a href={`/a/${agentId}/inbox`}>Inbox</a>{" · "}
        <a href={`/api/a/${agentId}/export`}>Download CSV</a>
      </p>

      <form onSubmit={save}>
        <label>
          Console password (MAKER_PASSWORD)
          <br />
