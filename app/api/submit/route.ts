...
export async function POST(req: Request) {
  // If the sample link adds ?demo=1, skip storing & emailing completely.
  ...
  // In demo mode: pretend it worked but do nothing server-side.
  if (isDemo) {
    return NextResponse.redirect(new URL("/thank-you?demo=1", req.url), { status: 303 });
  }

  const answers: Record<string, any> = {
    full_name: fd.get("full_name")?.toString() || "",
    contact: fd.get("contact")?.toString() || "",
    email: fd.get("email")?.toString() || "",         // if present
    phone: fd.get("phone")?.toString() || "",         // if present
    ...
    // if you added home_types:
    home_types: fd.getAll("home_types").map(v => String(v)),
  };
